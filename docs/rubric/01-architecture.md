...
|----|-----------|-------|--------------|------------|
| **A1** | **Framework-free ports.** `libs/ports` has no imports from `@angular/*` or `rxjs`. | Must | ESLint restricted imports | Ports use plain types, `Promise`, and shared result types. |
| **A2** | **Nx module boundaries.** `@nx/enforce-module-boundaries` configured and run in CI. | Must | Automated (CI: lint) | e.g. `type:feature` cannot import `type:adapter`; only `type:port`. |
| **A3** | **Injection tokens.** Ports are consumed only via `InjectionToken` (or equivalent DI token). | Must | Automated (TS/ESLint) | Swap implementations in `app.config` without changing components. |
...
| **A11** | **Feature-sliced libraries.** Code grouped by domain (`features-auth`, `features-org`), not by technical type only. | Must | Nx workspace layout | Each feature lib owns its routes/components for that domain. |
| **A12** | **Design system boundary.** Features use primitives from `libs/ui`; shared patterns not re-styled ad hoc. | Must | Manual (code review) | Consistent Spartan/Helm (or your DS) usage. |
