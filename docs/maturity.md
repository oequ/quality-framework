...
| **Accessibility Partial (v1.2)** | **P1** Partial at L1 only with published remediation roadmap (audit findings + timeline + plan for axe CI). |
...
| **Requirements** | 100% **Pass** on all **Must** criteria in **Architecture**, **Security**, and **Angular** (demo-tier Partial on S5/P1 allowed only as documented above); mock adapters; `AGENTS.md` or equivalent |
...
**Additional L2 gates (v1.1 – v1.3):**

| Domain | Gate |
|--------|------|
| Angular | **NG1 + NG2 Pass** (zoneless) |
| Architecture | **A1 Pass** (abstract-class ports or documented equivalent) |
| TypeScript | **TS8** runtime config path documented for production deploy |
| Security | **S1 Pass** (HTTP CSP path, not demo meta-only); **S2 Pass**; **S5 Pass** (no demo localStorage without production session path) |
| Testing | **T1 Pass** with mock-adapter E2E covering auth + tenant isolation; **T8 Pass** on ports/adapters coverage |
| Performance & a11y | **P1 Pass** with **axe (or equivalent) in CI** on primary flows; **P11 Pass** |
| SaaS domain | **SaaS5 Pass** (onboarding, not blank dashboard) |
| UX | **U11 Pass** (dark mode); **U4 Pass** (skeletons on primary views) |
| Documentation | **D3 Pass** (Scorecard ≥ 6.5); **D6 Pass** (CODEOWNERS); **D10 Pass** (published `docs/QUALITY.md` with evidence) |
...
- Any level without **rubric version** (e.g. Quality Framework v1.3)
...
A ports-and-adapters Angular 21 monorepo with Playwright, mock adapters, and demo-tier security may land **650–780** — often **L1 by score** but not **L2** until NG1/NG2, S1/S2/S5, P1 axe CI, and T8 port coverage are Pass.
